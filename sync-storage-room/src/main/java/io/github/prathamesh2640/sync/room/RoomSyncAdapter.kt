 ...
 *
 * The state/delete writes go through raw SQL, so they are not tied to Room's
 * observation the way a `@Query` write is. The engine re-reads pending work each
 * run, so its correctness does not depend on it; a host observing the table via a
 * Room `Flow` sees these changes on Room's next invalidation cycle.
 *
...
public class RoomSyncAdapter<T : SyncableEntity>(
    ...
    override suspend fun purgeExpiredTombstones(retentionDays: Int): Int = withContext(ioDispatcher) {
        ...
        database.openHelper.writableDatabase
            ...
            .apply {
                bindString(1, SyncState.FAILED.name)
                bindLong(2, cutoff)
            }
            .executeUpdateDelete()
    }
